...
from discord import Forbidden, ui, Interaction, SelectOption
from discord.ext import commands
from discord.ext.commands import CheckFailure

from botutils import get_prefix, get_time, GetChoice, cln
...
class Factions(commands.Cog):
    def __init__(self, bot):
        ...
        # Data for .fight is a small static resource; the potentially large
        # mutable faction state is loaded in cog_load below.
    ...
    def _load_state(self):
        ...
        if path.isfile(self.path):
            with open(self.path, "r", encoding="utf-8") as f:
                dat = json.load(f)  # type: dict
                if "main" in dat:
                    factions = dat["main"]  # type: dict
                    for guild_id, guild_factions in list(factions.items()):
                        for faction, data in list(guild_factions.items()):
                            factions[guild_id][faction]["members"] = list(
                                set(data["members"])
                            )
                if "boosts" in dat:
                    boosts = dat["boosts"]  # type: dict
                    for key, values in list(boosts.items()):
                        for guild_id, data in list(values.items()):
                            for faction, end_time in list(data.items()):
                                if time() > end_time:
                                    del boosts[key][guild_id][faction]
                if "notifs" in dat:
                    notifs = dat["notifs"]  # type: list
        return factions, boosts, notifs

    async def cog_load(self):
        self.factions, self.boosts, self.notifs = await asyncio.to_thread(
            self._load_state
        )
    ...
    async def cog_unload(self):
        def save():
            ...
            # os.replace is atomic and overwrites the destination on Windows.
    ...
    async def collect_claims(self, guild_id, faction=None) -> dict:
        ...
        guild_factions = self.factions[guild_id]
        active_guards = self.land_guard.get(guild_id, {})
        guarded_claims = set()
        now = time()
        for guarded_faction, expires_at in list(active_guards.items()):
            if guarded_faction not in guild_factions or now > expires_at:
                del active_guards[guarded_faction]
                continue
            guarded_claims.update(
                int(claim) for claim in guild_factions[guarded_faction]["claims"]
            )

        async def claims(faction) -> dict:
            """ returns claims & their data """
            fac_claims = {}
            if faction not in guild_factions:
                return fac_claims
            fac = guild_factions[faction]
            for claim in list(fac["claims"]):
                await asyncio.sleep(0)
                channel = self.bot.get_channel(int(claim))
                if not isinstance(channel, discord.TextChannel):
                    fac["balance"] += 250
                    with suppress(ValueError):
                        fac["claims"].remove(claim)
                    continue

                fac_claims[int(claim)] = {
                    "faction": faction,
                    "guarded": (
                        faction in active_guards and int(claim) in guarded_claims
                    ),
        ...
        global_claims = {}
        for faction in list(guild_factions):
            await asyncio.sleep(0)
            fac_claims = await claims(faction)  # type: dict
            global_claims.update(fac_claims)
    ...
    @commands.command(name="gib", description="Gives faction points to a member")
    @commands.cooldown(2, 60, commands.BucketType.guild)
    @commands.is_owner()
    async def gib(self, ctx, guild_id: str, faction: str, amount: int):
        """ Dev only command to give money """
        if guild_id not in self.factions:
            return await ctx.send("That server doesn't have factions")
        guild = self.bot.get_guild(int(guild_id))
        if faction not in self.factions[guild_id]:
            return await ctx.send(f"That faction doesn't exist in {guild}")
        self.factions[guild_id][faction]["balance"] += amount
        await ctx.send(f"Gave **{faction}** ${amount} in **{guild}**")

    @commands.command(name="taek", description="Takes faction points from a member")
    @commands.cooldown(2, 60, commands.BucketType.guild)
    @commands.is_owner()
    async def taek(self, ctx, guild_id: str, faction: str, amount: int):
        """ Dev only command to give money """
        if guild_id not in self.factions:
            return await ctx.send("That server doesn't have factions")
        guild = self.bot.get_guild(int(guild_id))
        if faction not in self.factions[guild_id]:
            return await ctx.send(f"That faction doesn't exist in {guild}")
        self.factions[guild_id][faction]["balance"] -= amount
        await ctx.send(f"Took ${amount} from **{faction}** in **{guild}**")
    ...
    async def create(self, ctx, *, name):
        ...
        if len(name) < 3:
            return await ctx.send("That name's too short, like you")
        if len([c for c in name if c.lower() == c.upper() and c != " "]) > 2:
            return await ctx.send("There's too many unicode characters in that name")
    ...
    async def transfer(self, ctx, *, user: discord.Member):
        ...
        if "yes" in msg.content.lower():
            ...
            await ctx.send("Ok, I won't transfer")
    ...
    async def kick(self, ctx, *, user: discord.User):
        ...
        if user.id == self.factions[guild_id][faction]["owner"]:
            return await ctx.send("You cant kick the owner ._.")
    ...
    async def _battle(self, ctx, user: discord.User, amount=50):
        ...
        e = discord.Embed(color=discord.Color.red())
        e.title = f"{cln(ctx.author.name)} Vs. {cln(user.name)}"
        ...
        while True:
            ...
            if health1 <= 0:
                await msg.edit(content=f"🏆 **{cln(user.name)} won** 🏆")
                self.factions[guild_id][fac1]["balance"] -= amount
                self.factions[guild_id][fac2]["balance"] += amount
                return await ctx.send(f"⚔ **{cln(user.name)}** has won **${amount}** from **{cln(ctx.author.name)}**")
            if health2 <= 0:
                await msg.edit(content=f"🏆 **{ctx.author.name} won** 🏆")
                self.factions[guild_id][fac2]["balance"] -= amount
                self.factions[guild_id][fac1]["balance"] += amount
                return await ctx.send(f"⚔ **{cln(ctx.author.name)}** has won **${amount}** from **{cln(user.name)}**")
            ...
            if attacker == 1:
                formatted = attack.replace('!user', cln(ctx.author.name)).replace('!target', cln(user.name))
                health2 -= dmg
            else:
                formatted = attack.replace('!user', cln(user.name)).replace('!target', cln(ctx.author.name))
                health1 -= dmg
            ...
            e.set_footer(text=f"{cln(ctx.author.name)} {health1}HP | {cln(user.name)} {health2}HP")
    ...
    async def work(self, ctx):
        ...
        if "compounding" in self.factions[guild_id][faction]["features"]:
            bonus = round(self.factions[guild_id][faction]["balance"] * 0.00025)
            e.description += f"\n+ ${bonus} compounding bonus"
            pay += bonus
    ...
    async def _scrabble(self, ctx):
        ...
        if scrambled_word == list(word[1:-1]):
            random.shuffle(scrambled_word)
    ...
    async def coin_flip(self, ctx, amount: int):
        ...
        async with ctx.channel.typing():
            msg = await ctx.send(f"**{cln(ctx.author.name)}** flipped a coin anddd.. 🤞")
    ...
    async def _vote(self, ctx):
        ...
        async with self.bot.utils.cursor() as cur:
            await cur.execute(
                "select vote_time from votes where user_id = %s "
                "order by vote_time asc;",
                (ctx.author.id,),
            )
            results = await cur.fetchall()
        if not results:
            return await ctx.send(f"Vote at {self.bot.vote_url} to earn $250."
                                  f"\nRerun the command after to redeem")
        async with self.bot.utils.cursor() as cur:
            await cur.execute(
                "delete from votes where user_id = %s "
                "order by vote_time asc limit 1;",
                (ctx.author.id,),
            )
    ...
    async def shop(self, ctx):
        faction = await self.get_owned_faction(ctx)
        if not faction:
            return await ctx.send("Only a faction owner can use this")
        view = Shop(self, ctx, faction).view
        msg = await ctx.send("🛍 | Choose an item to buy", view=view)
        await view.wait()
        await msg.edit(view=None)

    @factions.command(name="glb", description="Shows the global faction leaderboard")
    async def _glb(self, ctx):
        all_factions = [
            (name, data["balance"])
            for factions in self.factions.values()
            for name, data in factions.items()
        ]
        e = discord.Embed()
        e.set_author(
            name="Global Leaderboard", icon_url=self.bot.user.display_avatar.url
        )
        e.set_thumbnail(url=self.bot.user.display_avatar.url)
        e.description = ""
        for i, (name, bal) in enumerate(sorted(all_factions, key=lambda kv: kv[1], reverse=True)[:16]):
            e.description += f"\n**#{i + 1}.** `{name}` - **${bal}**"
        await ctx.send(embed=e)
...
class Shop(ui.Select):
    shelf: Dict[str, Dict[str, Any]] = {
        "Extra Slots": {
            "cost": 250,
            "emoji": "👥",
            "description": "Increase the max members you can have"
        },
        "Extra Income": {
            "cost": 50,
            "emoji": "💵",
            "description": "Gives extra income from channels & working for 2h"
        },
        "Anti Raid": {
            "cost": 75,
            "emoji": "🛡",
            "description": "Prevents .f raid being used on you for 48h"
        },
        "Land Guard": {
            "cost": 100,
            "emoji": "🚩",
            "description": "Prevents your claims from being taken for 2h"
        },
        "Time Chamber": {
            "cost": 500,
            "emoji": "⏱",
            "description": "Stacks work cmds while inactive, max of 3, for 4h"
        },
        "+25% Income": {
            "cost": 10000,
            "emoji": "💰",
            "description": "Gives extra income from channels & working",
            "children": ["+50% Income", "+75% Income"]
        },
        "+50% Income": {
            "cost": 25000,
            "emoji": "💰",
        ...
        "Compounding": {
            "cost": 125000,
            "emoji": "💸",
            "description": "Get 0.025% of your balance each time you work"
        }
    }

    def __init__(self, cls: Factions, ctx: commands.Context, faction: str):
        self.cls = cls
        self.guild_id = str(ctx.guild.id)
        self.user_id = ctx.author.id
        self.faction = faction
        self.data: dict = cls.factions[self.guild_id][self.faction]

        options = []
        for item, data in self.shelf.items():
            if parent := data.get("parent", None):
                if parent not in self.data["features"]:
                    continue
                if any("%" in s for s in self.data["features"]):
                    if item == "+25% Income":
                        continue
            options.append(SelectOption(
                emoji=data["emoji"],
                label=item + f" - ${data['cost']}",
    ...
    async def callback(self, interaction: Interaction):
        ...
        item: str = interaction.data["values"][0]
        cost: int = self.shelf[item]["cost"]
        ...
        if "Slots" in item:
            if self.cls.factions[self.guild_id][self.faction]["slots"] == 25:
                return await interaction.response.send_message(
                    "You're at the max number of slots",
                    ephemeral=True
                )
            self.cls.factions[self.guild_id][self.faction]["balance"] -= cost
            self.cls.factions[self.guild_id][self.faction]["slots"] += 5

        elif "Income" in item:
            if self.guild_id not in self.cls.extra_income:
                self.cls.boosts["extra-income"][self.guild_id] = {}
            self.cls.boosts["extra-income"][self.guild_id][self.faction] = time() + 60 * 60 * 2

        elif "Raid" in item:
            if self.guild_id not in self.cls.anti_raid:
                self.cls.boosts["anti-raid"][self.guild_id] = {}
            self.cls.boosts["anti-raid"][self.guild_id][self.faction] = time() + 60 * 60 * 12

        elif "Guard" in item:
            if self.guild_id not in self.cls.land_guard:
                self.cls.boosts["land-guard"][self.guild_id] = {}
            self.cls.boosts["land-guard"][self.guild_id][self.faction] = time() + 60 * 60 * 2

        elif "Time" in item:
            if self.guild_id not in self.cls.boosts["time-chamber"]:
                self.cls.boosts["time-chamber"][self.guild_id] = {}
            self.cls.boosts["time-chamber"][self.guild_id][self.faction] = time() + 60 * 60 * 4

        elif "Compounding" in item:
            self.cls.factions[self.guild_id][self.faction]["features"].append("compounding")

        else:
            return await interaction.response.send_message(
                "This item isn't available yet", ephemeral=True
            )

        self.cls.factions[self.guild_id][self.faction]["balance"] -= cost
        await interaction.response.send_message(
            f"Purchased {item} for ${cost}"
        )
        await self.cls.save_data()


async def setup(bot):
    await bot.add_cog(Factions(bot), override=True)
