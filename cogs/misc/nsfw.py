...
class NSFW(commands.Cog):
    ...
    @commands.command(name="gel", description="Searches Gelbooru for an image")
    ...
    async def _gel(self, ctx, *, tag):
        ...
        try:
            ...
            if not dat:
                return await ctx.send("No results found for that tag")
            if send_all:
                try:
    ...
    @commands.command(name="neko", description="Fetches a random neko image")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.guild_only()
    @commands.is_nsfw()
    @commands.bot_has_permissions(embed_links=True)
    ...
    @commands.command(name="yaoi", description="Fetches a random yaoi image")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.guild_only()
    @commands.is_nsfw()
    @commands.bot_has_permissions(embed_links=True)
...
async def setup(bot):
    await bot.add_cog(NSFW(bot), override=True)
