...
class Lock(commands.Cog):
    ...
    async def lockdown(self, ctx, *channels: discord.TextChannel):
        ...
        await ctx.send("Beginning lockdown")
        async with ctx.channel.typing():
            self.lock[guild_id][lock] = {}
            for channel in channels:
                await asyncio.sleep(0)
                if guild_id not in self.lock or lock not in self.lock[guild_id]:
            ...
            for channel_id, overwrites in self.lock[guild_id]["lockdown"].items():
                ...
                for object_id, perms in overwrites.items():
                    ...
                    permissions = discord.PermissionOverwrite(**{
                        perm: value for perm, value in perms if perm != "use_slash_commands"
                    })
...
async def setup(bot):
    await bot.add_cog(Lock(bot), override=True)
