...
class ModMail(commands.Cog):
    ...
    async def enable(self, ctx: Context):
        ...
        await self.bot.create_log(
            message=f"!on **Mod Mail** - `{ctx.guild}`",
            channel="module_log",
            embedded=True,
            color="green"
        )
    ...
    async def disable(self, ctx: Context):
        ...
        await self.bot.create_log(
            message=f"!off **Mod Mail** - `{ctx.guild}`",
            channel="module_log",
            embedded=True,
            color="red"
        )
    ...
    @commands.command(
        name="close-all",
        aliases=["closeall"],
        description="Closes all active modmail threads",
    )
...
async def setup(bot: Fate) -> None:
    await bot.add_cog(ModMail(bot), override=True)
