...
import discord
from botutils import colors, notable_permissions
    ...
    async def on_member_remove(self, member):
        ...
        if guild_id in self.guilds:
            ...
            for role in [role for role in member.roles if role and is_modifiable(role)]:
                if (
                    not any(perm in notable_permissions for perm in role.permissions)
...
async def setup(bot):
    await bot.add_cog(RestoreRoles(bot), override=True)
