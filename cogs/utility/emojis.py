...
class Emojis(commands.Cog):
    ...
    def record_actor(self, ctx, action: str, target) -> None:
        logger = self.bot.get_cog("Logger")
        if logger:
            logger.record_emoji_actor(ctx.guild.id, action, target, ctx.author)
    ...
    async def _upload_emoji(self, ctx, name, img, reason, roles=None) -> str:
        ...
        # Varying causes to fail
            ...
            # Emoji limit reached
            if "maximum" in error and "failed to resize" not in error:
                return f"{name} - Emoji Limit Reached"

            if "256" not in error and "failed to resize" not in error:
                return f"{name} - {err}"
    ...
    async def _delemoji(self, ctx, *emoji: discord.Emoji):
        for target_emoji in emoji:
            if target_emoji.guild.id != ctx.guild.id:
                await ctx.send(f"{target_emoji} doesn't belong to this server")
                continue
            self.record_actor(ctx, "delete", target_emoji.id)
            await target_emoji.delete(reason=ctx.author.name)
            await ctx.send(f"Deleted emote `{target_emoji.name}`")
    ...
    async def _rename_emoji(self, ctx, emoji: discord.Emoji, name):
        old_name = emoji.name
        self.record_actor(ctx, "create", name)
        try:
            self.record_actor(ctx, "rename", emoji.id)
            await emoji.edit(name=name.strip(":"), reason=ctx.author.name)
        except discord.errors.HTTPException:
            return await ctx.send("That's not a valid emoji name")
        await ctx.send(f"Renamed emote `{old_name}` to `{name}`")
    ...
    async def add_sticker(self, ctx, *args):
        ...
        converted_attachments = await asyncio.gather(
            *(attachment.to_file() for attachment in ctx.message.attachments)
        )
        files = [
            [attachment.filename, converted]
            for attachment, converted in zip(
                ctx.message.attachments, converted_attachments
            )
        ]
...
async def setup(bot: Fate):
    await bot.add_cog(Emojis(bot), override=True)
