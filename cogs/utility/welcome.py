...
class Welcome(commands.Cog):
    ...
    def record_activity(self):
        ...
        if telemetry is not None:
            telemetry.increment("welcome_leave_messages")
    ...
    async def _enable(self, ctx):
        ...
        await self.bot.create_log(
            message=f"!on **Welcome Msgs** - `{ctx.guild}`",
            channel="module_log",
            embedded=True,
            color=colors.green
        )
    ...
    async def _disable(self, ctx):
        ...
        await self.bot.create_log(
            message=f"!off **Welcome Msgs** - `{ctx.guild}`",
            channel="module_log",
            embedded=True,
            color=colors.red
        )
    ...
    async def on_member_join(self, member: discord.Member, just_verified=False):
        if not just_verified and (
            self.cd.check(str(member.guild.id) + str(member.id))
            or self.global_cd.check(member.guild.id)
        ):
            return
        if isinstance(member.guild, discord.Guild):
            ...
            if guild_id in self.config and self.config[guild_id]["enabled"]:
                ...
                msg = conf["format"] \
                    ...
                    .replace("!count", str(len(guild.members)))
...
async def setup(bot):
    await bot.add_cog(Welcome(bot), override=True)
