...
class Leave(commands.Cog):
    ...
    async def _test(self, ctx):
        ...
        msg = msg.replace("$MENTION", m.mention).replace("$SERVER", m.guild.name).replace("!mention", m.mention)
    ...
    async def on_member_remove(self, m: discord.Member):
        ...
        if isinstance(m.guild, discord.Guild):
            ...
            if guild_id in self.toggle:
                ...
                msg = msg.replace("!mention", m.mention)
                msg = msg.replace("!user", str(m)).replace("!server", m.guild.name)
...
async def setup(bot):
    await bot.add_cog(Leave(bot), override=True)
